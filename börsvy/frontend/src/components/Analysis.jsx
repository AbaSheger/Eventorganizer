        ...
        if (isMounted) {
          ...
          if (response.data.recentNews) {
            ...
            const formattedNews = response.data.recentNews.map(article => ({
              ...
              thumbnail: article.thumbnail || 'https://via.placeholder.com/150'
            }));
          ...
          {activeTab === 'analysis' ? (
            <div className="grid grid-cols-1 md:grid-cols-2 gap-6">
              ...
              <div className="bg-[#1a1a1a] rounded-xl shadow-sm p-6 border border-[#333333]">
                ...
                {isLoading ? (
                  <div className="animate-pulse h-32 bg-[#262626] rounded"></div>
                ) : error ? (
                  <div className="bg-red-500/10 p-4 rounded-lg border border-red-500/20">
                    <p className="text-red-400 text-base font-medium">{error}</p>
                  </div>
                ) : (
                  <div className="bg-[#262626] p-4 rounded-lg border border-[#333333]">
                    <p className="text-[#e6e6e6] text-base font-medium leading-relaxed whitespace-pre-line">
                      {analysis?.technicalAnalysis || 'Technical analysis not available'}
                    </p>
                  </div>
                )}
              </div>
              ...
              <div className="bg-[#1a1a1a] rounded-xl shadow-sm p-6 border border-[#333333]">
                ...
                {isLoading ? (
                  ...
                  <div className="bg-[#262626] p-4 rounded-lg border border-[#333333]">
                    <p className="text-[#e6e6e6] text-base font-medium leading-relaxed whitespace-pre-line">
                      {analysis?.fundamentalAnalysis || 'Fundamental analysis not available'}
                    </p>
                  </div>
                )}
              </div>
              ...
              <div className="bg-[#1a1a1a] rounded-xl shadow-sm p-6 border border-[#333333]">
                ...
                {isLoading ? (
                  <div className="animate-pulse h-20 bg-[#262626] rounded"></div>
                ) : error ? (
                  <div className="bg-red-500/10 p-4 rounded-lg border border-red-500/20">
                    <p className="text-red-400 text-base font-medium">{error}</p>
                  </div>
                  ...
                  <div className={`p-6 rounded-lg border ${
                    analysis?.sentiment === 'Bullish' || analysis?.sentiment === 'Very Bullish' 
                      ? 'bg-green-500/10 border-green-500/20' 
                    ...
                    <p className={`text-2xl font-bold leading-relaxed text-center ${
                      ...
                      {analysis?.sentiment || 'Sentiment not available'}
                    </p>
                  </div>
                )}
              </div>
