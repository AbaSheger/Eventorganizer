  ...
  useEffect(() => {
    ...
    const fetchChartData = async () => {
      ...
      try {
        setIsLoading(true);
        setError(null);
        const response = await axios.get(`http://localhost:8080/api/stocks/${symbol}/chart?interval=${interval}`);
        ...
        if (isMounted) {
          ...
          if (Array.isArray(response.data) && response.data.length > 0) {
            ...
            // Check if the data has the expected structure with prices and labels
            if (stockData.prices && stockData.labels && Array.isArray(stockData.prices) && Array.isArray(stockData.labels)) {
              formattedData = {
                labels: stockData.labels,
                prices: stockData.prices
              };
            } else {
              console.error('Unexpected stock data structure:', stockData);
              throw new Error('Unexpected stock data structure');
            }
          } else {
