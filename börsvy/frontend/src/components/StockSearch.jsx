...
function StockSearch({ onSearch, isLoading, popularStocks }) {
  ...
  const debouncedSearch = useRef(
    debounce(async (searchQuery) => {
      ...
      try {
        setIsSearching(true);
        const response = await axios.get(`http://localhost:8080/api/stocks/search?query=${searchQuery}`);
  ...
  const handleSearch = async () => {
    ...
    try {
      setIsSearching(true);
      const response = await axios.get(`http://localhost:8080/api/stocks/search?query=${query}`);
