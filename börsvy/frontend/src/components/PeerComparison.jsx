      ...
      <div className="bg-[#1a1a1a] p-6 rounded-xl border border-[#333333]">
        ...
        <div className="overflow-x-auto">
          <table className="min-w-full">
            <thead className="bg-[#262626] border-b border-[#333333]">
              <tr>
                <th className="px-6 py-3 text-left text-sm font-semibold text-gray-400">Company</th>
                <th className="px-6 py-3 text-right text-sm font-semibold text-gray-400">Price</th>
                <th className="px-6 py-3 text-right text-sm font-semibold text-gray-400">Change</th>
                <th className="px-6 py-3 text-right text-sm font-semibold text-gray-400">P/E Ratio</th>
                <th className="px-6 py-3 text-right text-sm font-semibold text-gray-400">Market Cap</th>
              </tr>
            </thead>
            <tbody className="divide-y divide-[#333333]">
              {peerData.peers.map((peer) => (
                <tr key={peer.symbol} className="hover:bg-[#262626] transition-colors">
                  <td className="px-6 py-4 whitespace-nowrap">
                    <div className="font-semibold text-[#e6e6e6]">{peer.name}</div>
                    <div className="text-sm text-gray-400">{peer.symbol}</div>
                  </td>
                  <td className="px-6 py-4 whitespace-nowrap text-right text-base font-semibold text-[#e6e6e6]">
                    ${peer.price?.toFixed(2) || 'N/A'}
                  </td>
                  <td className={`px-6 py-4 whitespace-nowrap text-right text-base font-bold ${
                    peer.changePercent >= 0 ? 'text-green-400' : 'text-red-400'
                  }`}>
                    {peer.changePercent >= 0 ? '+' : ''}{peer.changePercent?.toFixed(2) || 'N/A'}%
                  </td>
                  <td className="px-6 py-4 whitespace-nowrap text-right text-base font-semibold text-[#e6e6e6]">
        ...
        <div className="grid grid-cols-2 gap-6 mt-6">
          <div className="rounded-lg overflow-hidden shadow-sm border border-[#333333]">
            <div className="bg-[#262626] px-4 py-2 border-b border-[#333333]">
              <h3 className="text-blue-400 font-semibold">Industry Average P/E</h3>
            </div>
            <div className="p-4 bg-[#1a1a1a]">
              <div className="text-2xl font-bold text-[#e6e6e6]">{peerData.avgPE?.toFixed(2) || 'N/A'}</div>
