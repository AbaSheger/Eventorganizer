...
public class StockService {
    ...
    public Optional<Stock> getStockBySymbol(String symbol) {
        try {
            ...
            if (dbStock.isPresent()) {
                // Update cache
                stockCache.put(symbol, new CachedStock(dbStock.get()));
                return dbStock;
            }
        } catch (Exception e) {
    ...
    public Map<String, Object> getPeerComparison(String symbol) {
        ...
        try {
            ...
            List<Stock> industryPeers = stockRepository.findByIndustry(stock.getIndustry());
            ...
            for (Stock peer : industryPeers) {
                try {
                    StockDetails details = getStockDetails(peer.getSymbol());
                    if (details != null) {
                        peer.setPrice(details.getPrice());
                        peer.setChangePercent(details.getChangePercent());
                        peer.setPeRatio(details.getPeRatio());
                        peer.setMarketCap(details.getMarketCap());
                    }
                } catch (Exception e) {
